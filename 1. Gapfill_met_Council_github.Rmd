...
```{r}
ggplot(data = all)+
  geom_point(aes(date,st1,col='ERA5.1'))+ #upper soil layer - layer 1
  geom_point(aes(date,st2,col='ERA5.2'))+ #deeper soil layer - layer 2
...
#### ERA5 swc is VWC and is in decimals, needs to be x100 to get % 
```{r}
ggplot(data = all)+
  geom_point(aes(date,vwc1,col='ERA5.1'))+ #upper soil layer - layer 1
  geom_point(aes(date,vwc2,col='ERA5.2'))+ #deeper soil layer - layer 2
  geom_point(aes(date,all$SWC_1_1_1.c,col='Site1'))+
  geom_point(aes(date,all$SWC_2_1_1.c,col='Site2'))+
  geom_point(aes(date,all$SWC_3_1_1.c,col='Site3'))

  
all$vwc2.percent <- all$vwc2*100
all$vwc1.percent <- all$vwc1*100

#changed from SWC2 to SWC3 (tussock) & re-ran code on 6/16/2025
swc.mod = lm(formula = all$SWC_3_1_1.c ~ all$vwc1.percent)
summary(swc.mod) #VWC.1: R2 =  0.009353; slope =-0.41045
#VWC.2: R2 = -7^-6; slope = -0.01428 , p = 0.46

all$swc.eramod = all$vwc2.percent*swc.mod$coefficients[2] + swc.mod$coefficients[1]           

ggplot(data = all)+
  geom_point(aes(date,vwc2.percent,col='ERA5'))+
  geom_point(aes(date,SWC_3_1_1.c,col='Site'))+
  geom_point(aes(date,swc.eramod,col='Lin. Reg.'))


#make cleaner fig to show ERA5 and site soil temp agreement for paper

swc_plot <- ggplot(data = all, aes(x = vwc1.percent, y = SWC_3_1_1.c)) +
  theme_bw() +
  geom_point() +
  # Linear fit (blue solid line)
  geom_smooth(method = "lm",
  ...
  annotate("text",
           x = -10, y = 75,
           hjust = -0.1, vjust = 1.3,
           label = "R² =   0.009", size = 5) +
  annotate("text",
           x = -10, y = 70,
           hjust = -0.1, vjust = 1.3,
           label = "Slope = -0.41", size = 5) +
    #Title for the plot 
   labs(title = "Soil Moisture at 15 cm") 

print(swc_plot)


#modified ERA5
swc_modified = lm(formula = all$SWC_3_1_1.c ~ all$swc.eramod)
summary(swc_modified) #VWC.1: R2 =  0.009353; slope =-0.41045
#VWC2: R2 = -7.006e-06 , slope = 1

ggplot(data = all, aes(x = date, y = vwc2*100))+
  geom_point()

ggplot(data = all, aes(x = date, y = vwc1*100))+
  geom_point()


swc_comparison_plot  <- ggplot(data = all)+
  geom_point(aes(date,vwc1*100,col='ERA5'))+
   geom_point(aes(date,vwc2.percent,col='ERA5.2'))+
  geom_point(aes(date,SWC_3_1_1.c,col='Site')) +
...
```{r}
# Then save it
 ggsave("C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/combined_ERA5vsSite_met_variables2.png", final_plot, width = 20, height = 20, dpi = 600)
```
...
#Agreement between ERA5 and site measurements - pre-modification 
```{r}

...
df = fread(input = "./councilbase_met_flux_clean_2017_2023.csv",na.strings = c('-9999'))#redoing with re-cleaned data from Dani 
...
```{r}
write.csv(x = df,file = "./council_2016_2023.csv",row.names = F)
```

#Load in and Add ERA5 data
```{r}
#load the ERA5 df from step 0
...
#All look decent except for SWC which clearly is having a rough time
```
...
write.csv(x = df,file = './council_2016_2023_era.csv',row.names = F,quote = F)
```

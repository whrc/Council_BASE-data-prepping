...
```{r}
  #+++ Load data with one header and one unit row from (tab-delimited) text file
    EddyData.F <- fLoadTXTIntoDataframe('./reddy_council_2017_2023.txt')
...
ggplot(dat, aes(x = date, y = FC.c)) + geom_point()
summary(dat$FC.c)
...
```{r}
#this will not work if constrianing NEE in step 3 did not find a temp sensitivity 

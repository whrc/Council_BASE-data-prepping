...
summary(lm(test.data.nee$FC.c ~ test.data.nee$rfnee.2)) #R2 = 0.64, slope = 1.02
...
summary(lm(test.data.nee$FC.c ~ test.data.nee$rfnee.2)) #R2 = 0.65, slope = 1.02; p<0.001
...
summary(lm(test.data.nee$FC.c ~ test.data.nee$rf_model_1000)) #R2 = 0.65, slope = 1.02
...
summary(lm(test.data.ch4$FCH4.c ~ test.data.ch4$rfch4.2)) #R2 = 0.54, slope = 1.04; Residual standard error: 5.878 on 4441 degrees of freedom
...
councilch4.2= ggplot(data = test.data.ch4,aes(rfch4.2,FCH4.c))+theme_bw()+
  ...
  annotate(geom = 'text',x = 50, y = -8,label=expression(R^2~"= 0.54"),size = 3)+
  annotate(geom = 'text',x = 50,y = -15,label=expression(Slope~"= 1.04"),size = 3)+
  theme(text = element_text(size = 8))

councilch4.2 

#using wd and ntree = 1000
summary(lm(test.data.ch4$FCH4.c ~ test.data.ch4$rfch4.1000)) #R2 = 0.54, slope = 1.05; Residual standard error: 5.868 on 4441 degrees of freedom
...
councilch4.1000= ggplot(data = test.data.ch4,aes(rfch4.2,FCH4.c))+theme_bw()+
  ...
  scale_y_continuous(limits = c(-50,150),expression('Eddy Covariance FCH4 ('*mu*mol~CO[2]~m^-2~s^-1*')'))+
  annotate(geom = 'text',x = 50, y = -8,label=expression(R^2~"= 0.54"),size = 3)+
  annotate(geom = 'text',x = 50,y = -15,label=expression(Slope~"= 1.05"),size = 3)+
  theme(text = element_text(size = 8))
